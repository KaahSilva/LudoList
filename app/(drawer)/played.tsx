...
export default function PlayedScreen() {
  ...
  const removeFromPlayed = async (gameId: number) => {
    ...
    try {
      ...
      if (error) {
        console.error("Erro ao remover dos jogados:", error)
        return
      }
  ...
  const renderGame = ({ item }: { item: GameInList }) => {
    ...
    return (
      <GameCard
        ...
        onRemove={() => removeFromPlayed(item.game_id)}
        removeButtonText="Remover dos Jogados"
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text variant="bodyMedium" style={styles.emptyText}>
          Marque os jogos que você já jogou!
        </Text>
